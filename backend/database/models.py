...
class PlatformApiKey(Base):
    ...
    - Many bots may reference one key, within its transactionally checked capacity.
    ...
    __table_args__ = (
        CheckConstraint("max_bot_assignments >= 1", name="ck_platform_key_bot_capacity"),
    ...
    # Lifecycle status
    # "available" → enabled with no assignments
    # "assigned"  → enabled with assignments (not necessarily full)
    # "disabled"  → administratively disabled; cannot be allocated
    status = Column(String, default="available", nullable=False)

    max_bot_assignments = Column(Integer, default=2, server_default="2", nullable=False)

    # Historical reverse link only. Retained to avoid destructive schema changes;
    # no active code reads/writes it. Bot.platform_credential_id owns assignments.
    ...
    bots = relationship(
        "Bot", back_populates="platform_credential",
        foreign_keys="Bot.platform_credential_id", passive_deletes=True,
    )
