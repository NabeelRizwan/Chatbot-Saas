...
(:mod:`ragflow_derived.upstream.advanced_rag.agentic_rag_graph`) needs — question formalisation,
...
from ragflow_derived.full_runtime import Knowledgebase
from ragflow_derived.full_runtime import DocMetadataService
from ragflow_derived.full_runtime import DocumentService
from ragflow_derived.full_runtime import KnowledgebaseService
from ragflow_derived.model_runtime import AuthorizedChatModel as LLMBundle
from ragflow_derived.full_runtime import settings
from ragflow_derived.upstream.runtime import thread_pool_exec
from ragflow_derived.upstream.runtime import num_tokens_from_string
from ragflow_derived.upstream.advanced_rag.agentic_rag_graph import _split_think_stream
from ragflow_derived.upstream.advanced_rag.harness.keywords import extract_weighted_keywords
from ragflow_derived.upstream.advanced_rag.harness.stats import CountingChatModel, LLMUsageStats, in_phase, using_stats
from ragflow_derived.upstream.advanced_rag.harness.tools.search import _compact_keywords, _resolve_rerank_candidates, _resolve_top_k, _setting
from ragflow_derived.tagging import label_question
from ragflow_derived.upstream.tool_decorator import tool
from ragflow_derived.upstream.prompts.generator import (
    citation_prompt,
    form_message,
    gen_meta_filter,
    kb_prompt,
...
from typing import Any as WebSearchProvider
...
class RAGTools:
    ...
    def scoped_doc_ids(self, doc_scope: list[str] | None = None) -> list[str] | None:
        from ragflow_derived.full_runtime import scoped_documents
        return scoped_documents(doc_scope if doc_scope else self.doc_scope)
    ...
    async def structured_retrieve(self, question: str) -> dict[str, Any]:
        ...
        from ragflow_derived.full_runtime import use_sql
    ...
    def _filter_known_doc_ids(self, candidate_ids: list[str]) -> set[str]:
        if not candidate_ids or not self.kb_ids:
            return set()
        from ragflow_derived.full_runtime import scoped_documents
        return set(scoped_documents(candidate_ids))
