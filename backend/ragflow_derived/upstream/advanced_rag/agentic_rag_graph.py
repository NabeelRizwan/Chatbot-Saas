...
from ragflow_derived.upstream.advanced_rag.harness.config import NAIVE, resolve_mode
from ragflow_derived.upstream.advanced_rag.harness.stats import in_phase
from ragflow_derived.upstream.prompts.generator import form_message, kb_prompt, message_fit_in
...
def _select_sca_view(chunks: list, focus_terms: list[str], cap: int | None = None) -> tuple[list, str]:
    ...
    look at).
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _chunk_id
...
def _view_terms(state: AgenticState) -> list[str]:
    ...
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _query_to_terms
...
async def _expand_fanouts(tools, question: str, answer_conf: dict) -> list[str]:
    ...
    """
    try:
        from ragflow_derived.upstream.advanced_rag.harness.tools.search import _base_chat_mdl
    except Exception:  # noqa: BLE001
...
async def _fanout_search(tools, fanouts: list[str], top_n: int = 8, capacity: int | None = None) -> int:
    ...
    * Channel B (hybrid vector+BM25, **narrow bypass**): semantic hits whose
    ...
    """
    try:
        from ragflow_derived.upstream.advanced_rag.harness.grep_sed_narrow import narrow_by_terms
        from ragflow_derived.upstream.advanced_rag.harness.memory import _STOPWORDS as _FANOUT_STOPWORDS
        from ragflow_derived.upstream.advanced_rag.harness.tools.search import (
            _chunk_id,
            _get_kb_ids,
            _query_to_terms,
            bm25_search,
...
async def _direct_search_node_impl(state: AgenticState, tools) -> dict:
    from ragflow_derived.upstream.advanced_rag.harness.orchestrator.direct import direct_search
...
async def _compose_answer_from_evidence(state: AgenticState, tools, token_queue: asyncio.Queue, answer_conf: dict) -> dict:
    ...
    from ragflow_derived.upstream.advanced_rag.agentic_rag import _EVIDENCE_BUDGET_TOKENS
    ...
    if partial:
        from ragflow_derived.upstream.advanced_rag.harness.prompts.report_prompt import PARTIAL_ANSWER_PREAMBLE

        parts.append(f"{PARTIAL_ANSWER_PREAMBLE}\n")

    from ragflow_derived.upstream.advanced_rag.harness.prompts.report_prompt import FINAL_ANSWER_SYSTEM
    from ragflow_derived.upstream.prompts.generator import citation_prompt as cp
...
def build_agentic_graph(
    ...
    * The RAG-Agent research executor is slot-table-driven: the planner builds a
    ...
    async def sca(state: AgenticState) -> dict:
        from ragflow_derived.upstream.advanced_rag.harness.orchestrator.sufficient_context import sufficient_context_agent
    ...
    async def query_rewrite(state: AgenticState) -> dict:
        from ragflow_derived.upstream.advanced_rag.harness.orchestrator.query_rewriter import rewrite_gap_to_query
...
async def _build_slot_table(tools, question: str, fanouts: list, answer_conf: dict, deadline_left_fn=None) -> tuple:
    ...
    """
    from ragflow_derived.upstream.advanced_rag.harness.action_session import initialize_state
    ...
    if root is None or not root.state:
        ...
        from ragflow_derived.upstream.advanced_rag.harness.action_session import State, Variable
...
async def _run_slot_research_pass(tools, question: str, state: AgenticState, answer_conf: dict, deadline_left: float) -> dict:
    ...
    sessions.
    """
    from ragflow_derived.upstream.advanced_rag.harness.action_session import run_action_session
...
async def _compose_fallback_draft(tools, state: AgenticState, answer_conf: dict) -> str:
    ...
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _base_chat_mdl, _chunk_text
...
async def _naive_rag(tools, messages: list, gen_conf: dict | None = None):
    ...
    answer_conf = dict(gen_conf) if gen_conf else {"temperature": 0.3}
    try:
        from ragflow_derived.upstream.prompts.generator import form_message, message_fit_in
