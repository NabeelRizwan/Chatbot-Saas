...
from ragflow_derived.upstream.advanced_rag.harness.config import resolve_mode
...
# ── Tool executors ───────────────────────────────────────────────────────
def _kb_ids(tools):
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _get_kb_ids
...
def _admit_evidence(kbinfos, kb_seen, c, out, ids, seen, include_doc_id=True) -> bool:
    ...
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _chunk_id, _chunk_text, _doc_id, _is_table_chunk
...
async def _exec_retrieve(tools, queries: list, doc_scope: list | None = None, nav_hint: str = "") -> ToolOutcome:
    ...
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import grep_search
...
async def _exec_search_chunks(tools, queries: list, use_compiled: bool = False) -> ToolOutcome:
    ...
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import hybrid_search
...
async def _exec_list_chunks(tools, doc_id: str) -> ToolOutcome:
    ...
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _chunk_id, list_chunks
...
def _inject_nav_tools_ref(tools) -> None:
    ...
    """
    try:
        import ragflow_derived.upstream.advanced_rag.harness.tools.navigation as _nav
...
async def _exec_navigate_structure(tools, args: dict) -> ToolOutcome:
    ...
    to disable.
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.navigation import _navigate_structure_impl
...
async def _exec_calculate(tools, args: dict) -> ToolOutcome:
    ...
    from ragflow_derived.upstream.advanced_rag.harness.arithmetic import compute_from_facts
...
async def _exec_graph_explore(tools, args: dict) -> ToolOutcome:
    ...
    """
    from ragflow_derived.upstream.advanced_rag.harness.tools.navigation import graph_explore
...
async def _acompletion(mdl, messages: list, tools_list=None, temperature: float = 0.3, timeout_s: float = 60.0):
    ...
    response = await mdl.async_completion(oai_messages, tools=tools_list,
                                          temperature=temperature, timeout=timeout_s)
    from ragflow_derived.upstream.advanced_rag.harness.stats import record_external_response
...
def _extract_relevant_evidence(tools, direction: str, max_chunks: int = 4) -> str:
    ...
    from ragflow_derived.upstream.advanced_rag.harness.tools.search import _chunk_id, _chunk_text
...
async def run_action_session(
    ...
    from ragflow_derived.upstream.prompts.template import load_prompt
