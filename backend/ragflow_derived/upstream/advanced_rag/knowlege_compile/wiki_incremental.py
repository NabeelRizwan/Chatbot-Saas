...
from ragflow_derived.upstream.prompts.generator import message_fit_in
...
async def _load_map_relations(
    ...
    if chunk_state is None:
        from ragflow_derived.upstream.advanced_rag.knowlege_compile.wiki import _wiki_load_active_map_state

        chunk_state = await _wiki_load_active_map_state(tenant_id, kb_id)
    from ragflow_derived.upstream.advanced_rag.knowlege_compile.wiki import _wiki_load_map_extracts_for_state
    ...
    *,
    ...
    if mode == "delete":
        ...
        if not isinstance(deleted_count, int) or deleted_count <= 0:
            ...
            return existing_page
        from ragflow_derived.full_runtime import FileCommitService
