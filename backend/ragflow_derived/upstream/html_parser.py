...
from .runtime import decode_text, native_tokenizer
import logging
import re
import uuid
import chardet
...
class RAGFlowHtmlParser:
    ...
    def split_table(cls, html_table, chunk_token_num=512):
        ...
        table_str_list = []
        for row in rows:
            tks_str = cls.tokenizer.tokenize(str(row))
    ...
    def _token_count(cls, text):
        if not text:
            return 0
        # FIXME: The identity of the tokenizer behind `cls.tokenizer.tokenize`
        # (used here to size HTML chunks) is NOT confirmed. The earlier assumption
        # that it resolves to `infinity.cls.tokenizer.RagTokenizer` is incorrect.
        # Verify what `cls.tokenizer.tokenize` actually resolves to before claiming
        ...
        tks_str = cls.tokenizer.tokenize(text)
