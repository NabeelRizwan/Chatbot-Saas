...
from ..prompt_runtime import hash_str2int
from ..runtime import native_tokenizer as rag_tokenizer
from .template import load_prompt
from ..runtime import TAG_FLD
from ..prompt_runtime import get_encoder
...
async def full_question(tenant_id=None, llm_id=None, messages=[], language=None, chat_mdl=None):
    if chat_mdl is None:
        raise RuntimeError("EXPLICIT_CHAT_MODEL_REQUIRED")
    conv = []
    for m in messages:
...
async def gen_meta_filter(chat_mdl, meta_data: dict, query: str, constraints: dict = None) -> dict:
    ...
    try:
        ...
        logging.error("MODEL_JSON_PARSE_FAILED")

    return {"conditions": []}


async def gen_json(system_prompt: str, user_prompt: str, chat_mdl, gen_conf={}, max_retry=2):
    from ...model_runtime import get_llm_cache, set_llm_cache
...
async def table_of_contents_index(toc_arr: list[dict], sections: list[str], chat_mdl):
    ...
    for j, i in path:
        toc_arr[i]["indices"] = [j]
    logging.debug("TOC_ITEMS count=%s", len(toc_arr))
...
async def gen_toc_from_text(txt_info: dict, chat_mdl, callback=None):
    ...
    try:
        ...
        logging.error("MODEL_OPERATION_FAILED")
...
async def run_toc_from_text(chunks, chat_mdl, callback=None):
    ...
    try:
        ...
        logging.error("TOC_GENERATION_FAILED")
        for t in tasks:
            t.cancel()
    ...
    logging.debug("TOC_FILTER_COMPLETE")
    if not filtered:
        return []
    ...
    for item in toc_with_levels:
        if isinstance(item, dict):
            ...
            logging.warning("TOC_ITEM_INVALID type=%s", type(item).__name__)
...
async def gen_metadata(chat_mdl, schema: dict, content: str):
    ...
    if "properties" not in schema:
        logging.warning("METADATA_SCHEMA_PROPERTIES_MISSING")
        return ""
