...
from .doc_store import MatchTextExpr
from . import term_weight
from .runtime import native_tokenizer, NativeSynonyms


class FulltextQueryer(QueryBase):
    def __init__(self, tokenizer=native_tokenizer, synonyms=None):
        self.tokenizer = tokenizer
        self.tw = term_weight.Dealer(tokenizer)
        self.syn = synonyms if synonyms is not None else NativeSynonyms()
        self.query_fields = [
            "title_tks^10",
            "title_sm_tks^5",
            "important_kwd^30",
    ...
    def question(self, txt, tbl="qa", min_match: float = 0.6):
        ...
        if not self.tokenizer.tokenize(txt).strip():
            return None, []
        otxt = txt
        txt = self.rmWWW(txt)
        ...
        if not self.is_chinese(txt):
            ...
            for tk, w in tks_w[:256]:
                ...
                syn = [self.tokenizer.tokenize(s).replace("'", "") for s in self.syn.lookup(tk)]
                keywords.extend(syn)
        ...
        for tt in self.tw.split(txt)[:256]:  # .split():
            ...
            syns = " OR ".join(['"%s"' % self.tokenizer.tokenize(self.sub_special_char(s)) for s in syns])
            if syns and tms:
