...
GRAPH_RAPTOR_FAKE_DOC_ID = "__dataset__"
...
from ragflow_derived.upstream.raptor_utils import (
...
from types import SimpleNamespace as TaskContext
...
class RaptorService:
    ...
    async def _generate_raptor(
        ...
        """
        ctx = self._task_context
        from ragflow_derived.upstream.advanced_rag.knowlege_compile.raptor import RecursiveAbstractiveProcessing4TreeOrganizedRetrieval as Raptor
        ...
        if not is_tree:
            ...
            res = []
            tk_count = 0
            for idx, (content, vctr, source_chunk_ids, _) in enumerate(processed_chunks[original_length:], start=original_length):
                d = copy.deepcopy(doc)
