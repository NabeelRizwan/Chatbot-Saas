...
from . import query
import numpy as np
from .doc_store import MatchDenseExpr, FusionExpr, OrderByExpr, DocStoreConnection
from .string_utils import remove_redundant_spaces
from .float_utils import get_float
from .runtime import PAGERANK_FLD, TAG_FLD
from .tag_feature_utils import parse_tag_features
from .runtime import ES_SETTINGS

from .runtime import thread_pool_exec
...
class Dealer:
    ...
    def __init__(self, dataStore: DocStoreConnection, queryer=None, settings=ES_SETTINGS):
        self.settings = settings
        self.qryr = queryer if queryer is not None else query.FulltextQueryer()
    ...
    async def _existing_doc_ids(self, doc_ids: list[str]) -> set[str]:
        # Adapter rechecks authoritative scope; no cached MySQL tenant decisions.
        return set(self.dataStore.existing_doc_ids(doc_ids))
    ...
    async def search(self, req, idx_names: str | list[str], kb_ids: list[str], emb_mdl=None, highlight: bool | list | None = None, rank_feature: dict | None = None, min_match: bool = True):
        ...
        if not qst:
            ...
            if emb_mdl is None:
                ...
                if self.settings.DOC_ENGINE_OCEANBASE or self.settings.DOC_ENGINE_SERENEDB:
                    src.append(f"q_{len(q_vec)}_vec")

                if self.settings.DOC_ENGINE_INFINITY:
                ...
                elif self.settings.DOC_ENGINE_GAUSSDB:
            ...
            for k in keywords:
                kwds.add(k)
                for kk in self.qryr.tokenizer.fine_grained_tokenize(k).split():
                    if len(kk) < 2:
                        continue
                    if kk in kwds:
                        continue
    ...
    def insert_citations(self, answer, chunks, chunk_v, embd_mdl, tkweight=0.1, vtweight=0.9):
        ...
        chunks_tks = [self.qryr.tokenizer.tokenize(self.qryr.rmWWW(ck)).split() for ck in chunks]
        cites = {}
        thr = 0.63
        while thr > 0.3 and len(cites.keys()) == 0 and pieces_ and chunks_tks:
            for i, a in enumerate(pieces_):
                sim, tksim, vtsim = self.qryr.hybrid_similarity(ans_v[i], chunk_v, self.qryr.tokenizer.tokenize(self.qryr.rmWWW(pieces_[i])).split(), chunks_tks, tkweight, vtweight)
                mx = np.max(sim) * 0.99
    ...
    def hybrid_similarity(self, ans_embd, ins_embd, ans, inst):
        return self.qryr.hybrid_similarity(ans_embd, ins_embd, self.qryr.tokenizer.tokenize(ans).split(), self.qryr.tokenizer.tokenize(inst).split())
    ...
    async def retrieval(
        ...
        if rerank_mdl and sres.total > 0:
            ...
            if self.settings.DOC_ENGINE_INFINITY:
                ...
                tsim = sim
                vsim = sim
            elif self.settings.DOC_ENGINE_OCEANBASE or self.settings.DOC_ENGINE_SERENEDB:
