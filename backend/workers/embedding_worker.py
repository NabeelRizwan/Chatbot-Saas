...
def execute_document_job(job_id: str, bot_id: int, organization_id: int | None, document_id: int) -> Dict[str, Any]:
    """
    Worker handler for document upload / text ingestion jobs.
    Validates tenant scope, enforces the state machine atomically, and orchestrates ingestion.
    """
    db = SessionLocal()
    try:
        ...
        # 2. State: PROCESSING
        transition_job_state(db, job_id, "processing", stage="processing", progress_percent=30)
        ...
        # 3. State: EMBEDDING
        transition_job_state(db, job_id, "embedding", stage="embedding", progress_percent=60)
        ...
        # 4. State: VALIDATING
        transition_job_state(db, job_id, "validating", stage="validating", progress_percent=90)

        # 5. State: READY or FAILED
        if processed_doc.status in ("ready", "completed") or processed_doc.processing_status == "completed":
            ...
            err_code, safe_msg = sanitize_customer_error(
                Exception(processed_doc.processing_error or "Document processing failed")
            )
            transition_job_state(
                db,
                job_id,
